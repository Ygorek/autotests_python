...
from locators.elements_page_locators import TextBoxPageLocators, CheckBoxPageLocators, RadioButtonPageLocators, \
    ...
    DynamicPropertiesloadPageLocators
...
class LinksPage(BasePage):
    ...
    def check_new_tab_simple_link(self):
        ...
        if request.status_code == 200:
            ...
            print(self.driver.current_url, "ошибка")
...
class UploadAndDownloadPage(BasePage):
    ...
    def upload_file(self):
        ...
        os.remove(path)
        text = self.element_is_present(self.locators.UPLOADED_RESULT).text.split('\\')[-1]
        return file_name.split('\\')[-1], text

        # assert_file_name = file_name.split('\\')[-1]
        # assert assert_file_name == text

    def download_file(self):
        link = self.element_is_present(self.locators.DOWNLOAD_FILE).get_attribute('href')
        link_b = base64.b64decode(link)
        path_name_file = rf'D:\autotests_python_2\filetest{random.randint(0,999)}.jpg'
...
class DynamicPropertiesloadPage(BasePage):

    locators = DynamicPropertiesloadPageLocators()

    def check_enable_button(self):
        try:
            self.element_is_clickable(self.locators.ENABLE_BUTTON, 6)
